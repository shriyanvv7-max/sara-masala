...
export function HomePage({ featuredProducts }: { featuredProducts: Product[] }) {
  ...
  return <main>
    ...
    <section className="categories section" id="shop"><SectionHeading eyebrow="SHOP BY MOOD" title="A spice for every story" body="From the dishes you grew up with to new favourites waiting to happen."/>
      <div className="category-grid">{categories.map(([name,copy,img],i) => <motion.a whileHover={{y:-8}} href="#products" className="category-card" key={name}><Image src={img} alt="" width={736} height={1104} sizes="(max-width: 760px) 88vw, 31vw"/><div><span>0{i+1}</span><h3>{name}</h3><p>{copy}</p><b>Discover →</b></div></motion.a>)}</div>
    </section>
    ...
    <section className="story section" id="our-story"><div className="story-photo"><Image src="/images/references/whole-spices.jpg" alt="A colourful collection of whole Indian spices" width={540} height={800} sizes="(max-width: 760px) 88vw, 50vw"/><span>EST. 2026<br/>MYSURU</span></div><div className="story-copy"><p className="eyebrow">FROM OUR KITCHEN</p><h2>Made with memories,<br/><em>shared with love.</em></h2><p>At Sara, every blend begins with a memory of home: the dry roast before dawn, the unmistakable scent of curry leaves, and Paati's instinct for the perfect pinch.</p><p>We honour those rituals in small batches, so every meal feels a little more like it was made for you.</p><a href="/about" className="text-link">Meet the family behind Sara <b>→</b></a></div></section>

    <section className="recipes section" id="recipes"><SectionHeading eyebrow="FROM THE SARA TABLE" title="Cook something memorable"/><div className="recipe-grid">{recipes.map(([name,time,img]) => <motion.article whileHover={{y:-6}} className="recipe-card" key={name}><Image src={img} alt={name} width={736} height={736} sizes="(max-width: 760px) 88vw, 31vw"/><div><span>RECIPE · {time}</span><h3>{name}</h3><a href="/recipes">View recipe <b>→</b></a></div></motion.article>)}</div><div className="center"><PrimaryButton href="/recipes">Cook with Sara</PrimaryButton></div></section>
