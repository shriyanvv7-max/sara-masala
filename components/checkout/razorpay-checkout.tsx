...
export function RazorpayCheckout({ order, onSuccess, onFailure, onExpired }: { order: PaymentOrder; onSuccess: (number: string, token: string) => void; onFailure: (message: string) => void; onExpired: () => void }) {
  const [ready, setReady] = useState(false); const [busy, setBusy] = useState(false); const [response, setResponse] = useState<PaymentResponse | null>(null); const [expired, setExpired] = useState(() => new Date(order.expiresAt).getTime() <= Date.now()); const lock = useRef(false); const received = useRef(false);
